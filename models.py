...
class MyPositionEmbedding(nn.Module):
    def __init__(self, d_model, d_inner=None):
        super().__init__()
        if d_inner is None:
            d_inner = d_model // 16
...
class Attention(nn.Module):
    def __init__(self, embed_dim, num_heads, dropout=0, bias=True):
        ...
        self.q_pos = MyPositionEmbedding(d_model=self.embed_dim)
        self.k_pos = MyPositionEmbedding(d_model=self.embed_dim)
...
class MoEFFN(nn.Module):
    ...
    def forward(self, x):
        ...
        # 专家块
        expert_inputs = x.view(-1, x.shape[-1])  # 展平出所有token [batch_size*seq_len, hidden_size]
